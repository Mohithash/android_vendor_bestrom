# BestROM absolute host toolchain (auto)
KERNEL_ARCH ?= arm64
KERNEL_MAKE_CMD := prebuilts/build-tools/linux-x86/bin/make
KERNEL_MAKE_FLAGS += ARCH=$(if $(KERNEL_ARCH),$(KERNEL_ARCH),arm64)
KERNEL_MAKE_FLAGS += HOSTCC=prebuilts/clang/host/linux-x86/clang-r584948b/bin/clang
KERNEL_MAKE_FLAGS += HOSTCXX=prebuilts/clang/host/linux-x86/clang-r584948b/bin/clang++
KERNEL_MAKE_FLAGS += HOSTLD=ld.lld HOSTAR=llvm-ar
KERNEL_MAKE_FLAGS += PAHOLE=true

# BestROM ARCH default
KERNEL_ARCH ?= arm64
KERNEL_MAKE_CMD := prebuilts/build-tools/linux-x86/bin/make
MAKE := prebuilts/build-tools/linux-x86/bin/make
# BestROM clang safety
ifeq ($(strip $(TARGET_KERNEL_CLANG_PATH)),)
  TARGET_KERNEL_CLANG_PATH := prebuilts/clang/host/linux-x86/clang-r584948b
endif
KERNEL_MAKE_CMD := prebuilts/build-tools/linux-x86/bin/make
...
## Internal variables
COLLECT_MODULE_DEPS_CMD := python3 $(BUILD_TOP)/vendor/bestrom/scripts/collect-kernel-module-deps/collect-kernel-module-deps.py --non-interactive
...
ifneq ($(TARGET_KERNEL_CLANG_COMPILE),false)
    ...
    PATH_OVERRIDE += PATH=$(BUILD_TOP)/prebuilts/clang/host/linux-x86/clang-r584948b/bin:$$PATH
    PATH_OVERRIDE += PATH=$(BUILD_TOP)/prebuilts/clang/host/linux-x86/llvm-binutils-stable/bin:$$PATH
    ifeq ($(KERNEL_CC),)
        # BestROM: never emit CC=" clang" when CCACHE_BIN empty; use PATH clang from TARGET_KERNEL_CLANG_PATH
        ifneq ($(CCACHE_BIN),)
            KERNEL_CC := CC=prebuilts/clang/host/linux-x86/clang-r584948b/bin/clang LD=ld.lld AR=llvm-ar NM=llvm-nm OBJCOPY=llvm-objcopy STRIP=llvm-strip AR=llvm-ar NM=llvm-nm OBJCOPY=llvm-objcopy STRIP=llvm-strip AR=llvm-ar NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump READELF=llvm-readelf STRIP=llvm-strip
        else
            KERNEL_CC := CC=prebuilts/clang/host/linux-x86/clang-r584948b/bin/clang LD=ld.lld AR=llvm-ar NM=llvm-nm OBJCOPY=llvm-objcopy STRIP=llvm-strip AR=llvm-ar NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump READELF=llvm-readelf STRIP=llvm-strip
        endif
    endif
endif
...
# BestROM: host tools with clang (no system gcc); disable pahole/BTF
KERNEL_MAKE_FLAGS += PAHOLE=true
# PAHOLE=true makes pahole-flags.sh no-op-ish; also disable BTF via config
KERNEL_MAKE_FLAGS += CONFIG_DEBUG_INFO_BTF=n CONFIG_DEBUG_INFO_BTF_MODULES=n
...
$(PATH_OVERRIDE) prebuilts/build-tools/linux-x86/bin/make $(KERNEL_MAKE_FLAGS) -C $(KERNEL_SRC) O=$(KERNEL_BUILD_OUT_PREFIX)$(1) ARCH=$(
endef
...
$(PATH_OVERRIDE) prebuilts/build-tools/linux-x86/bin/make $(KERNEL_MAKE_FLAGS) -C $(TARGET_KERNEL_EXT_MODULE_ROOT)/$(1) M=$(2)/$(1) KERNEL_SRC=$(BUILD_TOP)/$(KERNEL_SRC) OUT_DIR=$(KERNEL_BUILD_OUT_PREFIX)$(KERNEL_OUT) O=$(KERNEL_BUILD_OUT_PREFIX)$(KERNEL_OUT) ARCH=$(
endef
...
$(PATH_OVERRIDE) prebuilts/build-tools/linux-x86/bin/make $(KERNEL_MAKE_FLAGS) -C $(BUILD_TOP)/$(KERNEL_SRC) M=$(2)/$(1) O=$(KERNEL_BUILD_OUT_PREFIX)$(KERNEL_OUT) ARCH=$(
endef
...
ifeq ($(BOARD_USES_QCOM_MERGE_DTBS_SCRIPT),true)
	...
	PATH=$(abspath $(HOST_OUT_EXECUTABLES)):$${PATH} python3 $(BUILD_TOP)/vendor/bestrom/build/tools/merge_dtbs.py --base $(DTBS_BASE) --techpack $(DTB_OUT)/arch/$(KERNEL_ARCH)/boot/dts/vendor/ --out $(DTBS_OUT)
